...
func createDetector(cfg config.ServerConfig) (*phidetector.PhiAccrualFailureDetector, error) {
	return phidetector.NewBuilder().
		...
		SetMaxSampleSize(cfg.MaxSampleSize).
		SetMinStdDeviationMillis(cfg.MinStdDeviationMillis).
		SetAcceptableHeartbeatPauseMillis(cfg.AcceptableHeartbeatPauseMillis).
		SetFirstHeartbeatEstimateMillis(cfg.FirstHeartbeatEstimateMillis).
		Build()
}
